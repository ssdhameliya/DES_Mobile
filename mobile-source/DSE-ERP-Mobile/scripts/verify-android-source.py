...
checks={
 ...
 '10.0.26 baseline': 'SERVER_BASELINE = "10.0.26"' in (root/'shared/src/commonMain/kotlin/org/dse/mobile/core/config/MobileBuildInfo.kt').read_text(),
 'mobile 1.2.24': 'FALLBACK_MOBILE_VERSION_NAME = "1.2.24"' in (root/'shared/src/commonMain/kotlin/org/dse/mobile/core/config/MobileBuildInfo.kt').read_text(),
...
for field in ['minimumSupportedDesktopVersion','latestDesktopVersion','minimumSupportedAndroidVersion','latestAndroidVersion','minimumSupportedIosVersion','latestIosVersion','environment','databaseName','utcTime','dateFormat','timePolicy']:
    ...
    print(f'10.0.26 runtime health field {field}: {"OK" if ok else "FAIL"}')
    failed |= not ok
