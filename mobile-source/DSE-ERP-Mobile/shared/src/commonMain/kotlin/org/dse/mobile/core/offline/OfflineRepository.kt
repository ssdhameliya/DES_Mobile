 ...
 * Writes are never queued after an ambiguous transport/decode failure because server v10.0.5
        ...
        val result: ApiResult<*> = try {
            when (item.kind) {
                OfflineMutationKind.CREATE_SALE, OfflineMutationKind.CREATE_PURCHASE, OfflineMutationKind.CREATE_FINANCE ->
                    return "${item.label} is a legacy offline CREATE. v1.2.4 will not replay it without a server idempotency key. Verify ERP and recreate the document manually if it is not present."
